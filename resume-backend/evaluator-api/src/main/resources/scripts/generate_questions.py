...
def main():
    """Main execution function to run the 3-step question generation."""
    if len(sys.argv) != 3:
        print("Error: Invalid arguments. Script requires a resume and a job description.", file=sys.stderr)
        sys.exit(1)

    if not API_KEY or API_KEY == "YOUR_API_KEY_HERE":
        print("Error: Gemini API key is not configured in generate_questions.py. Please add your key.", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        # STEP 1: Analyze the Job Description (Reused)
        ...
        # STEP 2: Analyze the Resume (Reused)
        ...
        # STEP 3: Generate Interview Questions (New)
        prompt3_template = load_file(os.path.join(script_dir, 'prompt_step3_question_generation.txt'))
        ...
        final_questions = call_gemini_api(prompt3)
        
        print(final_questions)

    except Exception as e:
        print(f"A critical error occurred during question generation: {e}", file=sys.stderr)
        sys.exit(1)
